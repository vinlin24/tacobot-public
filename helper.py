...
from names import REAL_NAMES, MUDAE_ID, BORG_ID, LH_ID, TSC_ID, VIN_ID
...
def get_logger(name: str, level: int = 10) -> tuple[logging.Logger, Callable, Callable, Callable, Callable, Callable]:
	"""
	Creates a logger with the program-wide formatter.
	Returns a 6-tuple: the logger and 5 logging functions corresponding to the conventional
	severity levels (logger.debug() ... logger.critical()) with decorated functionality.
	"""
	logger = logging.getLogger(name)
	logger.setLevel(level)

	# Don't need datefmt because Papertrail add-on keeps track of time (in local tz too)
	...
	logger.critical = partial(logger.critical, exc_info=True) # Always display traceback

	return (logger, logger.debug, logger.info, logger.warning, logger.error, logger.critical)

logger, *funcs = get_logger(__name__, 10)
# Abbreviate print functions
print, printinf, printwar, printerr, printcrt = funcs

##### GLOBAL CONSTANTS #####

SENSITIVE_KEYS: Final = ("TACOBOT_TOKEN",
						 "AWS_ACCESS_KEY",
						 "AWS_SECRET_KEY",
						 "CURRENCYSCOOP_KEY")
...
async def sendvin(bot: commands.Bot, content: str, header: str = None) -> None:
	...
	printinf("Sent message to vin dm_channel")

async def reactremove(bot: commands.Bot, message: discord.Message, delete_emoji: str = "🗑",
	timeout: Optional[float] = None, member: discord.Member = None) -> bool:
	"""
	...
	try:
		...
		printinf(f"Deleted message on reaction in #{message.channel}, {message.guild}")
		return True
	# Timed out
	except asyncio.TimeoutError:
		printinf(f"Message delete option timed out for message: {message.jump_url}")
		return False
...
def make_embed(description: str = None, title: str = None, color: str = "gold") -> discord.Embed:
	...
	# Invalid color name, resort to default
		...
		printwar(f"Invalid color name '{color}' passed into make_embed()")
		discord_color = "gold"

	embed = discord.Embed(color=discord_color, title=title or "", description=description or "")
	return embed
...
def rms_rgb(image: "PIL.Image.Image") -> Optional[tuple[int, int, int]]:
	"""
