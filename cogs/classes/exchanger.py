...
class Exchanger(object):
	...
	def __init__(self, api_key: str, path: str) -> None:
		...
		self.__path = path
		printinf(f"Initialized Exchanger object with json path set to: {path}")
	...
	async def update_rates(self, *, loop: Optional[asyncio.AbstractEventLoop] = None) -> bool:
		...
		printwar("Request to currencyscoop.com was attempted. Usage is limited to 5000 requests/mo")
		
		# Exit as to not overwrite existing data in json file
		if not response.ok:
			printerr(f"Response code {response.status_code}: {response.reason}")
			return False
		...
		printinf(f"Updated {self.path} with latest rates")
		return True
	...
	async def convert(self, amount: float, org_currency: str, new_currency: str = "USD", force_update: bool = False, *,
		...
		if force_update or self.rates is None:
			...
			if not await self.update_rates(loop=loop):
				raise requests.RequestException("unsucessful request in update_rates()")

		try:
			org_rate = self.rates[org_currency]
			new_rate = self.rates[new_currency]
		except KeyError as E:
			raise LookupError(f"unrecognized currency abbreviation '{E}'") from None
