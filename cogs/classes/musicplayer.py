...
import helper
from cogs.classes.songs import Song, SongQueue
...
class MusicPlayer(object):
    ...
    RELOAD_INTERVAL: Final = timedelta(hours=5)

    DOWNLOAD_FAILED_MSG: Final = \
    """
    ...
    > **3)** HTTP Error 429: YouTube banned my IP! Notify <@275263271253311489>.
    """
    ...
    def format_queue_page(self, start: int) -> str:
        ...
        """
        segment = self.song_queue.segment(start, start+9) # SongQueue w/ len 10
    ...
    async def queue_listener_loop(self, message: discord.Message, pages: list[discord.Embed], current_index: int) -> None:
        ...
        async def update_arrows() -> None:
            ...
            has_emoji = lambda emoji: any(str(rxn) == emoji for rxn in message.reactions)
        ...
        check = lambda rxn, user: rxn.message == message and not user.bot
        try:
            async with timeout(self.REACTPAGE_TIMEOUT):
                ...
                while True:
                    ...
                    if emoji == "🔄":
                        ...
                        # init_index formula
                        current_index = (min(self.pos, self.numtracks) - 1) // 10
                    elif emoji == "⬆":
                        # Don't go below 0
    ...
    def set_embed_footer(self, embed: discord.Embed) -> None:
        ...
        states = [] # list[tuple[str, str]]: each item is the (description, emoji)

        # Get the discord.Member object for bot
        bot_as_member = discord.utils.get(self.guild.members, id=self.bot.user.id)
 
        if self.vc is None or not self.vc.is_connected():
            states.append(("disconnected", "👋"))
        if bot_as_member.voice is not None and bot_as_member.voice.mute:
            states.append(("muted", "🔇"))
        if self.vc is not None and (self.vc.is_paused() or self.should_be_paused):
    ...
    def progress_msg(self, current: int, total: int) -> str:
        ...
        if current == total:
            outstr = "✅"
            printinf(f"Finished loading in queue")
        else:
            outstr = "⌛"
    ...
    async def reload_song(self, song: Song) -> None:
        ...
        if datetime.now() - song.creation > self.RELOAD_INTERVAL:
            printinf(f"Attempting to reload song {song.title}")
            msg = await self.ctx.send(embed=helper.make_embed(f"⏳ Reloading {song}..."))
            if song.reload():
                printinf(f"SUCCESS: Reloaded {song.title}")
            else:
                printerr(f"FAILED to reload {song.title}")
            try:
                await msg.delete()
            except discord.NotFound:
                pass
    ...
    async def cancel_loading(self, by: discord.Member) -> None:
        ...
        if self.loadqueue_task is not None and not self.loadqueue_task.done():
            ...
            printinf(f"{by} canceled current loadqueue task")
    ...
    async def player_loop(self) -> None:
        ...
        while self.vc is not None and self.vc.is_connected():
            ...
            try:
                ...
                printinf(f"MusicPlayer for guild {self.guild} timed out")
            ...
            try:
                ...
                self.vc.play(song.audiosource, after=lambda error: self.increment_pos())
            ...
            printinf(f"Now playing ({self.pos}) {song.title}")
    ...
    async def play(self, ctx, query: Optional[str], *, from_loadqueue=False) -> None:
        ...
        # Download failed
        if song is None:
            printwar(f"Failed to download song from query: {query}")
            desc = self.DOWNLOAD_FAILED_MSG.format(name=ctx.author.name, query=query)
        ...
        if not from_loadqueue and (self.vc.is_playing() or self.vc.is_paused()):

            printinf(f"Queued ({self.numtracks}) {song.title}")
    ...
    async def leave(self, ctx, *, by_timeout: bool = False) -> None:
        ...
        printinf(f"{self.guild} MusicPlayer disconnected from channel")

        if by_timeout:
            embed = helper.make_embed("❗ I left the voice channel because I was inactive for too long.")
            await ctx.send(embed=embed)
        else:
            await ctx.message.add_reaction("👋")
    ...
    async def skip(self, ctx) -> None:
        ...
        try:
            printinf(f"Stopped ({self.pos}) {self.song_queue.at(self.pos).title}")
        except IndexError:
            printinf(f"Skipped nothing, outside of the queue already (pos={self.pos})")
        await ctx.message.add_reaction("👌")
        self.ctx = ctx
    ...
    async def back(self, ctx) -> None:
        ...
        try:
            printinf(f"Stopped ({self.pos+2}) {self.song_queue.at(self.pos+2).title}")
        except IndexError:
            printinf(f"Skipped nothing, outside of the queue already (pos={self.pos})")
    ...
    async def jump(self, ctx, request: Union[int, str]) -> None:
        ...
        try:
            printinf(f"Stopped ({old_pos}) {self.song_queue.at(old_pos).title}")
        except IndexError:
            printinf(f"Skipped nothing, outside of the queue already (pos={old_pos})")
        self.ctx = ctx
    ...
    async def clear(self, ctx, *, disconnected: bool = False) -> None:
        ...
        printinf(f"Cleared {num_cleared} song(s) from queue")
    ...
    async def remove(self, ctx, request: Union[int, str]) -> None:
        ...
        printinf(f"Removed ({pos}) {removed_song.title}")
    ...
    async def removerange(self, ctx, pos1: int, pos2: int) -> None:
        ...
        printinf(f"Removed {num_removed} song(s) ({pos1}~{pos1+num_removed-1}) from queue")
    ...
    async def shuffle(self, ctx) -> None:
        ...
        await ctx.message.add_reaction("🔀")
        printinf(f"Shuffled {max(0, self.numtracks-self.pos)} tracks (after pos {self.pos})")
    ...
    async def savequeue(self, ctx, queue_name: Optional[str]) -> None:
        ...
        localpath = "files/temp_playlist.txt"

        # Ensure directory exists
        await self.s3_client.create_folder("tacobot", filedir)
        # Download playlists.txt if exists
        if not await self.s3_client.download(f"{filedir}playlists.txt", "tacobot", "files/temp_playlist.txt"):
            open("files/temp_playlist.txt", "w").close()
            printinf(f"{ctx.author} does not have a playlists.txt yet; using local file")
        ...
        printinf(f"{ctx.author} saved current queue as {queue_name} to personal list")
    ...
    async def loadqueue(self, ctx, queue_name: str, *, append: bool = False) -> None:
        ...
        if append:
            printinf(f"{ctx.author} is adding a queue: {existing_name}")
            ...
            printinf(f"{ctx.author} is loading in a queue: {existing_name}")
        ...
        check = lambda rxn, user: rxn.message == self.loading_msg and str(rxn) == "❌" and not user.bot
        rxn_aw = asyncio.create_task(self.bot.wait_for("reaction_add", check=check))
        self.loadqueue_task = asyncio.create_task(self.queue_songs(ctx, song_ids, embed_desc))
    ...
    async def showqueues(self, ctx) -> None:
        ...
        names_to_len = {name: len(SongQueue.get_saved_contents(localpath, name)[1]) for name in names}
        
        desc = f"**Playlists PREVIEW** [{ctx.author.mention}]\n\n"
        body = "\n".join(f"**{name}**: {len_} songs" for name, len_ in names_to_len.items())
