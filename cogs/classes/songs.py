...
class Song(object):
    ...
    def duration_str(self) -> str:
        ...
        seconds = self.duration % (24 * 3600)   # If more than a day, take the remainder
        hours = seconds // 3600
        minutes = seconds // 60
        seconds %= 60
    ...
    def truncstr(self, max_chars: int =None) -> str:
        """
    ...
    def reload(self) -> bool:
        ...
        with self.ytdl as ytdl:
            try: 
                ...
                if "entries" in info:
                    info = info["entries"][0]
            # Known to catch "TypeError: argument of type 'NoneType' is not iterable"
            except Exception:
                return False
    ...
    def preview_str(cls, video_id: int) -> Optional[str]:
        ...
        try:
            with urllib.request.urlopen(url) as response:
                ...
                data = json.loads(response_text.decode()) # dict[str, Union[str, int]]
...
class SongQueue(object):
    ...
    VIDEO_ID_PATTERN: Final = r"[0-9A-Za-z_-]{10}[048AEIMQUYcgkosw]"
    QUEUE_PATTERN: Final = "\\{{{name}\\}}\\{{(\\n{video_id})*\\n?\\}}\\n"
    ...
    def __str__(self) -> str:
        ...
        return f"{{{self.name}}}{{\n" + "\n".join(str(song) for song in self) + "}"
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        try:
            return all(self.at(i) == other.at(i) for i in range(1, len(self)+1)) \
    ...
    def truncstr(self, max_chars: int =None) -> str:
        ...
        return f"({self.name})" + "\n{\n" + "\n".join(song.truncstr(max_chars) for song in self) + "\n}"
    ...
    def at(self, pos: int) -> Union[Song, NoReturn]:
        ...
        if pos < 1 or pos > len(self):
            raise IndexError(f"queue position {pos} out of range [1, len(self)]")
    ...
    def pop_song(self, pos: int) -> Union[Song, NoReturn]:
        ...
        song = self.at(pos) # Use this so it can appropriately raise custom IndexError
    ...
    def remove_song(self, name: str) -> Union[Song, NoReturn]:
        ...
        for song in self:
            ...
            raise KeyError(f"could not find Song whose title contains substring '{name}'")
    ...
    def swap_pos(self, pos1: int, pos2: int) -> Union[tuple[Song, Song], NoReturn]:
        ...
        swapped_songs = (self.at(pos1), self.at(pos2)) # Can raise custom IndexError
    ...
    def shuffle(self, pos: int =0) -> None:
        ...
        tail = self._queue[pos:] # Returns a copy; shuffle won't mutate self._queue
    ...
    def get_saved_contents(cls, filepath: str, name: str) -> Optional[tuple[str, list[str]]]:
        ...
        pattern = cls.QUEUE_PATTERN.format(name=name, video_id=cls.VIDEO_ID_PATTERN)
        re_obj = re.compile(pattern, re.IGNORECASE)     # Case insensitive matching
