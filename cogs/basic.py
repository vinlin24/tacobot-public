...
class Basic(commands.Cog):
	...
	async def ping(self, ctx) -> None:
		outstr = f"**[{self.bot.VERSION_STRING}]** Yes, I'm here! Bot latency: **{round(self.bot.latency*1000)}** ms"
		await ctx.send(outstr)

	@commands.command(name="version", aliases=["v"], help="Displays version of running script")
	async def version(self, ctx) -> None:
		await ctx.send(f"Script version: **{self.bot.VERSION_STRING}**")

	@commands.command(name="say", aliases=["echo"], help="Echoes what you say")
	async def say(self, ctx, *, content: str = None) -> None:
		# Attempt to delete user's message
		try:
			await ctx.message.delete()
		except (discord.Forbidden, discord.HTTPException):
			printwar(f"Couldn't delete user message in #{ctx.channel}, {ctx.guild}")
		if content is not None:
			await ctx.send(content)

	@commands.command(name="cleandm", aliases=["purgedm"], help="Deletes bot messages from your DM")
	async def cleandm(self, ctx, num_messages: int = 1) -> None:
		...
		printinf(f"Deleting up to {num_messages} message(s) from DM channel of {channel.recipient}...")
		...
		printinf(f"Deleted {num_deleted} message(s) from DM channel of {channel.recipient}")
		await ctx.message.add_reaction("✅")

	@commands.command(name="updatenotes", aliases=["updatenote"], help="Displays update notes for current script version")
	async def updatenotes(self, ctx) -> None:
		...
		pattern = r"(?is)\[" + self.bot.VERSION_STRING + r".*\].*$"
		match = helper.re.search(pattern, content)
		if match is None:
			await ctx.send(f"No update notes found for **{self.bot.VERSION_STRING}**!")
			return
