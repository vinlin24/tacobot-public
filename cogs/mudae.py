...
class Mudae(commands.Cog):
	...
	def datetime_formats(self) -> list[str]:
		...
		formats = ["%m/%d", "%m/%d", "%m-%d", "%m-%d", "%b %d", "%d %b %y", "%d %b"]
		for fmt in formats.copy():
			formats.extend((fmt+"/%Y", fmt+"/%y", fmt+"-%Y", fmt+"-%y", fmt+" %Y", fmt+" %y", fmt+", %Y", fmt+", %y"))
		for fmt in formats.copy():
	...
	async def update_json(self, guild_id: int, num_claimed: int) -> None:
		...
		with open("files/mudae_data.json", "rt+") as file:
		...
		await self.bot.s3_client.upload(
			"files/mudae_data.json", "tacobot", "mudae_data.json")
	...
	async def get_exchange_dets(self, message: discord.Message) -> Optional[tuple[discord.Member, str, discord.Member, str, discord.Message]]:
		...
		# No-break: one of response_match and confirm_match not found
			...
			printwar(f"get_exchange_dets() failed to parse history of #{message.channel}")
			return None
	...
	async def on_message(self, message: discord.Message) -> None:
		...
		if num_claimed is not None:
			printinf(f"$left message detected in #{message.channel}: updating mudae_data.json")
			await self.update_json(message.guild.id, num_claimed)

	### COMMANDS ###

	@commands.command(name="kakeravalue", aliases=["kv"], help="Calculates the kakera value of Mudae character")
	async def kakeravalue(self, ctx,
		...
		if chars_claimed is None:
			...
			await self.bot.s3_client.download(
				"mudae_data.json", "tacobot", "files/mudae_data.json")
	...
	@commands.command(name="exchanges", aliases=["trades"], help="Parses channel history and displays most recent $marryexchange's")
	async def exchanges(self, ctx, num_messages: int = 100, *, before: Optional[str] = None) -> None:
		...
		if before is not None:
			for fmt in self.datetime_fmts:
				try:
					...
					printinf(f"Converted param before to datetime: {before}")
					break
				except (ValueError, re.error):
					continue
			# Parsing failed: default to None
			else:
				printwar(f"Failed to convert '{before}' to a datetime object")
				before = None
		...
		async with ctx.typing():
			...
			# Skip overx rolls
			check = lambda m: m.author.id == MUDAE_ID and len(m.embeds) == 0
	...
	@commands.command(name="claims", help="Parses channel history and displays most recent claims")
