...
import helper
from cogs.classes.exchanger import Exchanger
...
class Utils(commands.Cog):
	...
	def __init__(self, bot: commands.Bot) -> None:
		...
		self.bot.repl_sessions = []  # list[tuple[int, int]]: list of (user ID, channel ID)
		...
		# For currency()
		self.exchanger = Exchanger(self.bot.CURRENCYSCOOP_KEY, "files/exchange_rates.json")

	def printer(self, *args: Any, sep: str = " ", end: str = "\n", file=None, flush: bool = False) -> None:
	...
	def pprinter(self, obj: Any, stream=None, indent: int = 1, width: int = 80, depth: Optional[int] = None, *, compact: bool = False, sort_dicts: bool = True) -> None:
		...
		"""
		self.text_to_print = helper.pformat(obj, indent, width, depth, compact=compact, sort_dicts=sort_dicts)
	...
	def evaluate(self, expression: str, namespace: dict[str, Any]) -> Union[Any, NoReturn]:
		...
		if any(str(getattr(self.bot, key)) in str(result) for key in helper.SENSITIVE_KEYS):
			return "⛔ Your code attempted to display a sensitive value(s)!"

		return result

	def execute(self, expression: str, namespace: dict[str, Any]) -> Optional[str]:
		"""
	...
	@commands.command(name="eval", aliases=["pythoneval"], help="Runs Python's eval() on expression")
	async def eval_(self, ctx, *, expression: str) -> None:
		...
		try:
			...
			# Prevent display of sensitive values
			if any(str(getattr(self.bot, key)) in result for key in helper.SENSITIVE_KEYS):
	...
	@commands.command(name="repl", aliases=["python"], help="Starts a Python REPL session in current channel")
	async def repl(self, ctx) -> None:
		...
		# Initial message
		printinf(f"User has started a REPL session ({ctx.author}, #{ctx.channel})")
		...
		while len(header) + len(code) + 6 + len(footer) <= 2000:
			...
			try:
				...
				printinf(f"REPL session timed out ({ctx.author}, #{ctx.channel})")
			...
			# Exit
			if instr == "exit()":
				printinf(f"User exited REPL session ({ctx.author}, #{ctx.channel})")
				footer = "You have exited the REPL session."
				break

			try:
				result = repr(self.evaluate(instr, namespace))	# Display repr
			except SyntaxError:
				result = self.execute(instr, namespace)  # result is error msg | None
		...
		# No-break: the message became too long to send
			...
			printwar(f"Message for REPL session became too long ({ctx.author}, #{ctx.channel})")
	...
	@commands.command(name="analyze", aliases=["rgb"], help="Calculates the root mean square RGB of most recent image")
	async def analyze(self, ctx) -> None:
		...
		async for message in ctx.channel.history(limit=100):
			...
			printinf(f"Attempting to calculate r.m.s RGB on image with url: {image_url}")
			break

		else:
			printinf(f"No images detected in most recent 100 messages of channel #{ctx.channel}")
			await ctx.message.add_reaction("❌")
			return

		# Create/overwrite temp file
		with open(f"files/image_to_analyze.png", "wb") as file:
			file.write(image_data)

		# Use PIL Image object
		image = Image.open(f"files/image_to_analyze.png")
		rmsrgb = helper.rms_rgb(image)

		# image is a GIF -> pixel values of image retrieved as int -> rms_rgb() returns None
		if rmsrgb is None:
			printwar(f"%analyze does not support analyzing GIFs")
		...
		embed = discord.Embed(color=discord.Color(int(hexcode, 16)), description=desc)
	...
	@commands.command(name="anagrams", aliases=["anagram"], help="Finds the anagrams of your word")
	...
	@commands.command(name="currency", help="Calculates amount of a currency in terms of another currency.")
	async def currency(self, ctx, amount: float, original: str, new: str = "USD", latest: bool = False):
		...
		last_updated = self.exchanger.last_updated.strftime("%b %d, %Y %H:%M:%S")
	...
	@commands.command(name="memberdetails", aliases=["memberdets", "member", "dets"], help="Gets details about a member")
