...
class Mathematics(commands.Cog):
	...
	def parse_matrix(self, expression: str) -> Union[Matrix, str]:
		...
		try:
			rows = [[convert(val) for val in row_str.split()] for row_str in row_strs]
		except ValueError:
	...
	@commands.command(name="rref", aliases=["gaussjordan"], help="Calculates the rref of the given matrix")
