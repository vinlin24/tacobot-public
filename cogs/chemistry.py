...
class Chemistry(commands.Cog):
	...
	def basic_info_dict(self, compound: pcp.Compound) -> dict[str, Union[int, str]]:
		...
		# Basic physical data
		try:
			data["Mol. Formula"] = compound.molecular_formula.translate(self.subscripts)
	...
	@commands.command(name="periodictable", aliases=["periodic", "ptable"], help="Displays the periodic table of the chemical elements")
	async def periodictable(self, ctx) -> None:
		embed = helper.make_embed(color="green")
		embed.set_image(url="https://upload.wikimedia.org/wikipedia/commons/thumb/0/03/Simple_Periodic_Table_Chart-blocks.svg/1920px-Simple_Periodic_Table_Chart-blocks.svg.png")
		out_msg = await ctx.send(embed=embed)
		await helper.reactremove(self.bot, out_msg)

	@commands.command(name="pubchem", aliases=["chem"], help="Searches query on PubChem and returns basic info if found")
	async def pubchem(self, ctx, *, query: str) -> None:
		...
		async with ctx.typing():
			...
			# No-break: query pulls no results
				...
				printinf(f"{ctx.author} searched for '{query}' on PubChem and pulled no results")
			...
			printinf(f"Processing result of PubChem query '{query}' for {ctx.author}...")
			# Use first result
			...
			func = partial(pcp.download, "PNG", "files/molecule.png", query, namespace, overwrite=True)
			await self.bot.loop.run_in_executor(None, func)

			# Generate URL for image so it can be used in an embed
			await self.bot.s3_client.upload("files/molecule.png", "tacobot", "molecule.png")
			...
			entries = [f"https://pubchem.ncbi.nlm.nih.gov/compound/{match.cid}", ""]
			...
			embed.set_footer(text=f"Search type: {namespace.upper()}\nQuery: \"{query}\"")
