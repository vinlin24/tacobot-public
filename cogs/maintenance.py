...
class Maintenance(commands.Cog):
	...
	async def setactivity(self, ctx, *, name: str = None) -> None:
		...
		printinf("Reset bot presence" if activity is None else f"Changed bot activity to: '{activity}'")

		# Save to file and update on AWS S3
		with open("files/activity.txt", "wt") as file:
			file.write("" if name is None else name)
		await self.bot.s3_client.upload("files/activity.txt", "tacobot", "activity.txt")

	@commands.command(name="scripteval", aliases=["seval", "scriptvar", "svar"], hidden=True)
	...
	@commands.command(name="playereval", aliases=["peval", "playervar", "pvar"], hidden=True)
	...
	async def restart(self, ctx) -> None:
		...
		if self.bot.ON_HEROKU:
			...
			embed = helper.make_embed("☁ Restarting script running on Heroku...", color="dark_red")
			await ctx.send(embed=embed)

			printinf(f"Restart call made by {ctx.author}")
			await self.bot.close()  # Raises a bunch of errors out of my control

	@commands.command(name="abort", aliases=["shutdown", "logout"], hidden=True)
	async def abort(self, ctx) -> None:
		...
		"""
		if not self.bot.ON_HEROKU:

			embed = helper.make_embed("💻 Aborting script running on local machine...", color="dark_red")
			await ctx.send(embed=embed)

			printinf(f"Abort call made by {ctx.author}")
			await self.bot.close()  # Raises a bunch of errors out of my control
